...
"""
Derived from stupid L3 switch to cache ip from all connected openflow switches 
...
4) When you see an IP packet, if you know the destination port (because it's
   in the table from step 1), forward packet to switch and port with matched
   dest ip.
"""
...
class l3_switch (EventMixin):
  ...
  def addArp (self, dpid, port, ip, mac):
    self.arpTable[ip] = Entry(dpid, port, mac)
  ...
  def _handle_openflow_PacketIn (self, event):
    ...
    if isinstance(packet.next, ipv4):
      ...
      if packet.next.srcip in self.arpTable and self.arpTable[packet.next.srcip] != (dpid, inport, packet.src):
      ...
      self.addArp(dpid, inport, packet.next.srcip, packet.src)
      ...
      if dstaddr in self.arpTable:
        ...
        prt = self.arpTable[dstaddr].port
        mac = self.arpTable[dstaddr].mac
        if dpid == self.arpTable[dstaddr].dpid and prt == inport:
          ...
          dest = self.arpTable[dstaddr]
          msg = of.ofp_packet_out(data = event.ofp.data,
            action = of.ofp_action_output(port = dest.port))
          core.openflow.sendToDPID(dest.dpid, msg)
      ...
      if a.prototype == arp.PROTO_TYPE_IP:
        if a.hwtype == arp.HW_TYPE_ETHERNET:
          if a.protosrc != 0:
            ...
            # Learn or update port/MAC info
            if a.protosrc in self.arpTable:
              if self.arpTable[a.protosrc] != (dpid, inport, packet.src):
            ...
            self.addArp(dpid, inport, a.protosrc, packet.src)
            ...
            if a.opcode == arp.REQUEST:
              ...
              if a.protodst in self.arpTable:
                ...
                if not self.arpTable[a.protodst].isExpired():
                  ...
                  r.hwsrc = self.arpTable[a.protodst].mac
